...
- **Semantic Consistency**: **동일한 설정(Config)은 어떤 언어/플랫폼에서 실행되든 동일한 의미(Semantics)와 동일한 HTTP 요청 결과를 보장해야 합니다.**
    - 구현체는 Config에 정의되지 않은 임의의 로직을 추가하여 HTTP 요청을 변경해서는 안됩니다. (예: 특정 Header가 존재할 때만 Request Body를 압축하는 등의 행위)
    - 이를 위반하는 경우, 동일한 코드가 환경에 따라 다르게 동작하는 문제를 야기할 수 있습니다.
...
- **Consistent Error Structure**: 라이브러리에서 발생하는 모든 예외는 [3. 에러 처리](3.ErrorHandling.md) 섹션에 정의된 표준 구조(코드, 메시지 등)를 따라야 합니다. 이를 통해 사용자는 언어에 상관없이 일관된 방식으로 에러를 처리할 수 있습니다.
